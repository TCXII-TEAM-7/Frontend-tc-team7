// Testing algorithm A_mirror with input 29283
...
// For loop - calculate mirror
console.log("\n--- For Loop (Building Mirror) ---");
for (let n = 0; n <= deg - 1; n++) {
    let extracted = A % (10 ** (n + 1));
    let shifted = extracted * (10 ** (deg - n));
    temp = shifted;
    MA = temp + MA;
    console.log(`n = ${n}: (${A} mod 10^${n+1}) * 10^${deg-n} = ${extracted} * ${10**(deg-n)} = ${shifted}, MA = ${MA}`);
    deg = deg - 1;
}
